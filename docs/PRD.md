...
| **双轨模式 (Dual-Track Mode)** | A 轨：改编编剧模式（AI 改写 + 审核流程）<br>B 轨：原文模式（跳过改写与审查，但需提取资产） |
...
- **输入**: 章节原文 + 全局项目状态
- **输出**: 原文直接作为剧本（无改写）
- **流程**: 原文 → 角色概念设计师 → 场景概念设计师 → 分镜执行导演
- **说明**: 跳过 AI 改编编剧和剧本医生，但必须进行资产提取，保证实体资产库的统一
...
#### 4.6.1 系统设置页面
- **功能点**:
  - 多服务商 API Key 配置管理（智谱 GLM、阿里千问、Google Gemini、OpenAI 等）
  - 任务路由策略配置（文本/图片/视频模型的动态选择）
  - 当前激活模型展示与切换
  - 模型调用统计与成本监控

- **UI 交互**:
  - 系统设置 → 模型配置 Tab
  - 卡片式展示各服务商的配置状态（已配置/未配置/连接测试）
  - 下拉菜单选择当前激活的模型：
    - 文本大模型：glm-4-plus、glm-4-long、qwen-max、gemini-pro、gpt-4-turbo 等
    - 生图模型：cogview-3、sd3、dalle-3 等
    - 生视频模型：sora2、runway-gen2 等
  - 实时显示模型健康状态（在线/离线/超时）

#### 4.6.2 API 凭证管理
- **功能点**:
  - 添加/编辑/删除服务商 API Key（加密存储）
  - 测试 API 连接性（一键验证）
  - 查看每个服务商的调用统计（调用次数、成功/失败率、累计费用）
  - 支持多实例配置（同一服务商配置多个 Key，自动负载均衡）

- **安全设计**:
  - API Key 加密存储（AES-256）
  - 敏感信息脱敏显示（仅展示前 4 位和后 4 位）
  - 操作日志审计（记录谁在何时修改了配置）

#### 4.6.3 动态路由策略
- **功能点**:
  - 按任务类型路由（文本/图片/视频）
  - 按项目级别路由（不同项目使用不同的模型组合）
  - 按用户偏好路由（高级用户可自定义路由规则）
  - 故障自动切换（主模型不可用时自动切换到备用模型）
